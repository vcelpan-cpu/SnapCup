...
async def help_cmd(message: Message):
    if is_admin(message.from_user.id):
        await message.answer(
            ...
            "<b>Заказы</b>\n"
            "📋 Заказы / /orders — сегодняшние заказы с кнопками принять/отменить\n"
            ...
            "Уведомления о новых заявках и заказах приходят сюда автоматически.\n"
    ...
    async with await get_session() as session:
        orders = (await session.execute(
            select(Order).where(
                ...
                Order.status.notin_([OrderStatus.locked.value, OrderStatus.cancelled.value]),
            )
        )).scalars().all()
        for order in orders:
