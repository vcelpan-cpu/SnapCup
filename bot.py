...
def admin_menu_kb() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="📋 Заказы"), KeyboardButton(text="📦 Отчёт")],
            [KeyboardButton(text="⋯ Ещё")],
        ],
        resize_keyboard=True,
    )
...
async def start(message: Message, state: FSMContext):
    ...
    if shop is None:
        ...
        await message.answer(
            "👋 <b>Привет!</b>\n"
            "Я бот приёма заказов кондитерской — беру на себя ежедневную рутину с кофейнями 🧁\n\n"
            "Как называется ваша кофейня? Напишите название одним сообщением, и начнём знакомство ✨"
        )
        return
    ...
    if order is not None and order.status != OrderStatus.draft.value:
        ...
        greeting = f"С возвращением, {shop.name}! 🧁\n\n📋 Заказ на сегодня: {label}"
    else:
        greeting = f"С возвращением, {shop.name}! 🧁\n\nЗаказ на сегодня ещё не оформлен."

    await message.answer(greeting, reply_markup=shop_menu_kb())
...
async def save_shop_address(message: Message, state: FSMContext):
    ...
    await message.answer("📞 И последнее: контактный телефон или имя ответственного за приём заказов?")
...
async def save_shop_contact(message: Message, state: FSMContext, bot: Bot):
    ...
    await state.clear()
    await message.answer("✅ Заявка отправлена!\nЖдите подтверждения — напишу сразу, как только одобрят 🙌")
...
async def cmd_order(message: Message):
    ...
    await message.answer(
        "🧾 <b>Собираем заказ на сегодня</b>\n"
        "Жмите ➖ / ➕ под каждой позицией, а затем «✅ Оформить».\n"
        "Передумаете — можно поменять сколько угодно раз до кат-оффа 🕐",
...
async def submit_order(callback: CallbackQuery, bot: Bot):
    ...
    header = "✏️ <b>Заказ изменён</b>" if was_submitted_before else "🆕 <b>Новый заказ</b>"
    item_lines = [f"▪️ {p.name} — {i.qty} {p.unit}" for i, p in rows]
    detail_lines = shop_detail_lines(shop)
    detail_text = ("\n" + "\n".join(detail_lines)) if detail_lines else ""
    await bot.send_message(
        ADMIN_CHAT_ID,
        f"{header}\n━━━━━━━━━━━━━━━\n☕ <b>{shop.name}</b>{detail_text}\n\n" + "\n".join(item_lines),
    )

    await callback.message.answer(
        "✅ <b>Заказ оформлен</b> и передан на кухню 🧁\n"
...
async def _contact_text() -> str:
    return CONFECTIONERY_CONTACT or "Свяжитесь с менеджером кондитерской напрямую по известному вам номеру."
...
class EditShopInfo(StatesGroup):
    waiting_address = State()
    waiting_contact = State()


def build_edit_shop_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="📍 Изменить адрес", callback_data="edit_shop_address"),
        InlineKeyboardButton(text="📞 Изменить контакт", callback_data="edit_shop_contact"),
    ]])


async def _my_info_content(shop: CoffeeShop) -> str:
    detail_lines = shop_detail_lines(shop)
    detail_text = "\n".join(detail_lines) if detail_lines else "Адрес и контакт не указаны."
    return f"✏️ <b>{shop.name}</b>\n{detail_text}\n\nЧто хотите изменить?"


@shop_router.message(F.text == "✏️ Мои данные")
async def my_info_button(message: Message):
    shop = await get_shop(message.from_user.id)
    if shop is None or shop.status != ShopStatus.approved:
        await message.answer("Сначала нужно подтверждение доступа. Отправьте /start.")
        return
    await message.answer(await _my_info_content(shop), reply_markup=build_edit_shop_kb())


@shop_router.callback_query(F.data == "edit_shop_address")
async def edit_shop_address_start(callback: CallbackQuery, state: FSMContext):
    await state.set_state(EditShopInfo.waiting_address)
    await callback.message.answer("📍 Новый адрес кофейни?")
    await callback.answer()


@shop_router.message(EditShopInfo.waiting_address)
async def edit_shop_address_finish(message: Message, state: FSMContext):
    shop = await get_shop(message.from_user.id)
    async with await get_session() as session:
        db_shop = await session.get(CoffeeShop, shop.id)
        db_shop.address = message.text.strip()[:256]
        await session.commit()
    await state.clear()
    await message.answer("✅ Адрес обновлён.", reply_markup=shop_menu_kb())


@shop_router.callback_query(F.data == "edit_shop_contact")
async def edit_shop_contact_start(callback: CallbackQuery, state: FSMContext):
    await state.set_state(EditShopInfo.waiting_contact)
    await callback.message.answer("📞 Новый контактный телефон или имя ответственного?")
    await callback.answer()


@shop_router.message(EditShopInfo.waiting_contact)
async def edit_shop_contact_finish(message: Message, state: FSMContext):
    shop = await get_shop(message.from_user.id)
    async with await get_session() as session:
        db_shop = await session.get(CoffeeShop, shop.id)
        db_shop.contact = message.text.strip()[:128]
        await session.commit()
    await state.clear()
    await message.answer("✅ Контакт обновлён.", reply_markup=shop_menu_kb())


async def _shop_stats_text(shop: CoffeeShop) -> str:
    async with await get_session() as session:
        rows = (await session.execute(
    ...
    if not rows:
        return "📊 Пока нет ни одного заказа в истории."
    ...
    return (
        "📊 <b>Ваша статистика</b>\n"
        "━━━━━━━━━━━━━━━\n"
        f"🧾 Всего заказов: <b>{len(order_dates)}</b>\n"
        f"📦 Всего позиций: <b>{total_items}</b>\n"
        f"🗓 В этом месяце: <b>{month_items}</b>"
    )


@shop_router.message(F.text == "📊 Статистика", IsNotAdmin())
async def shop_stats_button(message: Message):
    shop = await get_shop(message.from_user.id)
    if shop is None or shop.status != ShopStatus.approved:
        await message.answer("Сначала нужно подтверждение доступа. Отправьте /start.")
        return
    await message.answer(await _shop_stats_text(shop))


def build_shop_more_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📊 Статистика", callback_data="shop_more_stats"),
         InlineKeyboardButton(text="📞 Связь", callback_data="shop_more_contact")],
        [InlineKeyboardButton(text="✏️ Мои данные", callback_data="shop_more_info"),
         InlineKeyboardButton(text="❓ Помощь", callback_data="shop_more_help")],
    ])


@shop_router.message(F.text == "⋯ Ещё", IsNotAdmin())
async def shop_more_menu(message: Message):
    shop = await get_shop(message.from_user.id)
    if shop is None or shop.status != ShopStatus.approved:
        await message.answer("Сначала нужно подтверждение доступа. Отправьте /start.")
        return
    await message.answer("Выберите раздел:", reply_markup=build_shop_more_kb())


@shop_router.callback_query(F.data == "shop_more_stats")
async def shop_more_stats_cb(callback: CallbackQuery):
    shop = await get_shop(callback.from_user.id)
    await callback.message.answer(await _shop_stats_text(shop))
    await callback.answer()


@shop_router.callback_query(F.data == "shop_more_contact")
async def shop_more_contact_cb(callback: CallbackQuery):
    await callback.message.answer(f"📞 Связь с кондитерской:\n{await _contact_text()}")
    await callback.answer()


@shop_router.callback_query(F.data == "shop_more_info")
async def shop_more_info_cb(callback: CallbackQuery):
    shop = await get_shop(callback.from_user.id)
    await callback.message.answer(await _my_info_content(shop), reply_markup=build_edit_shop_kb())
    await callback.answer()


@shop_router.callback_query(F.data == "shop_more_help")
async def shop_more_help_cb(callback: CallbackQuery):
    await callback.message.answer(
        "☕ <b>Команды кофейни</b>\n\n"
        ...
        "✏️ Мои данные — изменить адрес или контактный телефон\n\n"
...
async def broadcast_start(message: Message, state: FSMContext):
    ...
    await message.answer("📢 Текст рассылки для всех подключённых кофеен?")


@admin_router.message(Broadcast.waiting_text)
async def broadcast_preview(message: Message, state: FSMContext):
    ...
    async with await get_session() as session:
        count = (await session.execute(
            select(func.count()).select_from(CoffeeShop).where(CoffeeShop.status == ShopStatus.approved)
        )).scalar_one()
    ...
    await message.answer(
        f"Отправить {count} кофейням следующее сообщение?\n\n«{message.text}»",
        reply_markup=kb,
    )
...
async def broadcast_confirm(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    for shop in shops:
        try:
            await bot.send_message(shop.tg_id, f"📢 {text_to_send}")
            sent += 1
        except Exception:
            pass

    await callback.message.edit_text(f"✅ Отправлено {sent} из {len(shops)} кофеен.")
    await callback.answer()
...
@admin_router.message(F.text == "❓ Помощь", IsAdmin())
async def help_cmd(message: Message):
    if is_admin(message.from_user.id):
        await message.answer(_admin_help_text(), reply_markup=admin_menu_kb())
        return
...
def build_admin_more_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="☕ Кофейни", callback_data="admin_more_shops"),
         InlineKeyboardButton(text="🍰 Каталог", callback_data="admin_more_catalog")],
        [InlineKeyboardButton(text="📊 Статистика", callback_data="admin_more_stats"),
         InlineKeyboardButton(text="➕ Товар", callback_data="admin_more_addproduct")],
        [InlineKeyboardButton(text="📢 Рассылка", callback_data="admin_more_broadcast"),
         InlineKeyboardButton(text="📄 CSV отчёт", callback_data="download_report_csv")],
        [InlineKeyboardButton(text="❓ Помощь", callback_data="admin_more_help")],
    ])


@admin_router.message(F.text == "⋯ Ещё", IsAdmin())
async def admin_more_menu(message: Message):
    await message.answer("Выберите раздел:", reply_markup=build_admin_more_kb())


@admin_router.callback_query(F.data == "admin_more_shops")
async def admin_more_shops_cb(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await _do_list_shops(callback.message.answer)
    await callback.answer()


@admin_router.callback_query(F.data == "admin_more_catalog")
async def admin_more_catalog_cb(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await _do_show_catalog(callback.message.answer)
    await callback.answer()


@admin_router.callback_query(F.data == "admin_more_stats")
async def admin_more_stats_cb(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await _do_admin_stats(callback.message.answer)
    await callback.answer()


@admin_router.callback_query(F.data == "admin_more_addproduct")
async def admin_more_addproduct_cb(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await state.set_state(AddProduct.waiting_name)
    await callback.message.answer("Название новой позиции?")
    await callback.answer()


@admin_router.callback_query(F.data == "admin_more_broadcast")
async def admin_more_broadcast_cb(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await state.set_state(Broadcast.waiting_text)
    await callback.message.answer("📢 Текст рассылки для всех подключённых кофеен?")
    await callback.answer()


@admin_router.callback_query(F.data == "admin_more_help")
async def admin_more_help_cb(callback: CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    await callback.message.answer(_admin_help_text())
    await callback.answer()
...
async def list_shops(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _do_list_shops(message.answer)
...
def build_catalog_admin_kb(products: list[Product]) -> InlineKeyboardMarkup:
    ...
    for p in products:
        ...
        rows.append([
            InlineKeyboardButton(text="💰 Цена", callback_data=f"edit_price:{p.id}"),
            InlineKeyboardButton(text="🗑 Удалить", callback_data=f"delete_product:{p.id}"),
        ])
    return InlineKeyboardMarkup(inline_keyboard=rows)


async def _do_show_catalog(answer):
    async with await get_session() as session:
        products = (await session.execute(select(Product).order_by(Product.sort_order))).scalars().all()
    if not products:
        await answer("Каталог пуст. Нажмите «➕ Товар», чтобы добавить первую позицию.", reply_markup=admin_menu_kb())
        return
    await answer(
...
async def show_catalog(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _do_show_catalog(message.answer)
...
async def delete_product_cb(callback: CallbackQuery):
    ...
    async with await get_session() as session:
        ...
        if used > 0:
            await callback.answer(
                "Позиция уже встречается в заказах — удалить нельзя (иначе сломается история). "
                "Выключите её кнопкой ✅/🚫 вместо удаления.",
                show_alert=True,
            )
            return
    ...
    if products:
        ...
        await callback.message.edit_text("Каталог пуст. Нажмите «➕ Товар», чтобы добавить первую позицию.")
...
async def edit_price_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(f"Новая цена для «{name}»?")
    await callback.answer()


@admin_router.message(EditPrice.waiting_price)
async def edit_price_finish(message: Message, state: FSMContext):
    ...
    async with await get_session() as session:
        ...
        if not product:
            await state.clear()
            await message.answer("Позиция уже не существует.")
            return
        product.price = price
        name = product.name
        await session.commit()
    await state.clear()
    await message.answer(f"💰 Цена «{name}» обновлена: {price}₽")
...
async def list_orders(message: Message):
    ...
    async with await get_session() as session:
        ...
        approved_shops_count = (await session.execute(
            select(func.count()).select_from(CoffeeShop).where(CoffeeShop.status == ShopStatus.approved)
        )).scalar_one()

    ordered_shop_ids = {shop.id for _, shop in orders}
    not_ordered_count = max(0, approved_shops_count - len(ordered_shop_ids))

    status_counts = defaultdict(int)
    for order, _ in orders:
        status_counts[order.status] += 1

    summary_lines = [f"📊 Статус заказов на {today().strftime('%d.%m.%Y')}:"]
    for status_value in ("submitted", "confirmed", "completed", "cancelled", "locked"):
        count = status_counts.get(status_value, 0)
        if count:
            summary_lines.append(f"{ORDER_STATUS_LABELS[status_value]}: {count}")
    if not_ordered_count:
        summary_lines.append(f"⏳ Ещё не заказали: {not_ordered_count}")

    if not orders:
        summary_lines.append("\nОформленных заказов пока нет.")
        await message.answer("\n".join(summary_lines), reply_markup=admin_menu_kb())
        return
...
async def _fetch_report_rows():
    async with await get_session() as session:
        rows = (await session.execute(
            ...
            .order_by(CoffeeShop.name, Product.sort_order)
        )).all()
    return rows
...
async def build_report_text() -> str | None:
    ...
    for order, shop, item, product in rows:
        ...
        line_sum = float(product.price) * item.qty
        grand_total_sum += line_sum
        by_shop[shop.name].append(f"    • {product.name}: {item.qty} {product.unit}")

    date_str = today().strftime("%d.%m.%Y")
    lines = [
        f"🧁 <b>Сводный заказ на {date_str}</b>",
        "━━━━━━━━━━━━━━━",
    ]
    for name, qty in sorted(totals.items()):
        lines.append(f"▪️ {name} — <b>{qty}</b>")
    lines.append(f"\n💵 Ориентировочная сумма: <b>{grand_total_sum:.0f}₽</b>")
...
async def build_report_csv_bytes() -> bytes | None:
    rows = await _fetch_report_rows()
    if not rows:
        return None
    ...
    writer.writerow(["Кофейня", "Позиция", "Количество", "Единица", "Цена", "Сумма"])
    for order, shop, item, product in rows:
        line_sum = float(product.price) * item.qty
        writer.writerow([shop.name, product.name, item.qty, product.unit, float(product.price), f"{line_sum:.2f}"])

    # utf-8-sig — чтобы Excel корректно показывал кириллицу без доп. настроек
    return buffer.getvalue().encode("utf-8-sig")


def build_report_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="📄 Скачать CSV", callback_data="download_report_csv"),
    ]])


@admin_router.callback_query(F.data == "download_report_csv")
async def download_report_csv_cb(callback: CallbackQuery, bot: Bot):
    if not is_admin(callback.from_user.id):
        await callback.answer("Только для админов", show_alert=True)
        return
    csv_bytes = await build_report_csv_bytes()
    if csv_bytes is None:
        await callback.answer("Заказов на сегодня пока нет", show_alert=True)
        return
    filename = f"report_{today().strftime('%Y-%m-%d')}.csv"
    await bot.send_document(
        callback.message.chat.id,
        BufferedInputFile(csv_bytes, filename=filename),
        caption=f"📄 Отчёт за {today().strftime('%d.%m.%Y')}",
    )
    await callback.answer()
...
async def lock_and_report(bot: Bot, admin_chat_id: int):
    ...
    if text_ is None:
        await bot.send_message(admin_chat_id, f"🌙 Заказов на {today().strftime('%d.%m.%Y')} не было.")
        return
    await bot.send_message(admin_chat_id, text_, reply_markup=build_report_kb())
...
async def manual_report(message: Message):
    ...
    text_ = await build_report_text()
    if text_ is None:
        await message.answer("Заказов на сегодня пока нет.", reply_markup=admin_menu_kb())
        return
    await message.answer(text_, reply_markup=build_report_kb())


# ---------- Статистика ----------

async def _do_admin_stats(answer):
    ...
    if not rows:
        await answer("📊 За последнюю неделю заказов не было.", reply_markup=admin_menu_kb())
        return
...
async def admin_stats(message: Message):
    if not is_admin(message.from_user.id):
        return
    await _do_admin_stats(message.answer)
